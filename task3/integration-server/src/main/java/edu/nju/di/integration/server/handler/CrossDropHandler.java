...
public final class CrossDropHandler implements HttpHandler {
    private final IntegrationConfig config;
    private final XsltTransformer xslt;
    private final HttpClient httpClient;

    public CrossDropHandler(IntegrationConfig config, XsltTransformer xslt) {
        this.config = config;
        this.xslt = xslt;
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            HttpXmlIO.writeXmlResponseUtf8(exchange, 500, errorXml("跨系退课失败: " + e.getMessage()));
        }
    }
    ...
    private String process(String requestXml) throws DocumentException, IOException, TransformerException, InterruptedException {
        ...
        if (targetDept == null || targetDept.isBlank()) {
            throw new IllegalArgumentException("缺少targetDept");
        }
        ...
        String forwardUrl = config.resolveCrossDropUrl(targetDept);
        if (forwardUrl == null) {
            throw new IllegalArgumentException("未配置目标院系的crossDropUrl: " + targetDept);
        }

        String forwardXml = "<crossDropRequest>" +
            tStudent.asXML() +
            tClass.asXML() +
            tChoice.asXML() +
            "</crossDropRequest>";
    ...
    private static String errorXml(String msg) {
        ...
        return "<crossDropResponse><success>false</success><message>" + safe + "</message></crossDropResponse>";
    }
