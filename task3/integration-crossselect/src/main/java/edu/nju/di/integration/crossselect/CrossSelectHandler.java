...
public final class CrossSelectHandler implements HttpHandler {
    private final CrossSelectConfig config;
    private final XsltTransformer xslt;
    private final HttpClient httpClient;

    public CrossSelectHandler(CrossSelectConfig config, XsltTransformer xslt) {
        this.config = config;
        this.xslt = xslt;
    ...
    private String process(String requestXml) throws DocumentException, IOException, TransformerException, InterruptedException {
        ...
        if (resp.statusCode() >= 400) {
            throw new IOException("院系B返回HTTP " + resp.statusCode() + ": " + resp.body());
        }
        return resp.body();
    }

    private String resolveForwardUrl(String targetDept) {
        return switch (targetDept) {
            case "A" -> {
                if (config.deptACrossSelectUrl == null || config.deptACrossSelectUrl.isBlank()) {
                    throw new IllegalArgumentException("未配置deptA.crossSelectUrl，但targetDept=A");
                }
                yield config.deptACrossSelectUrl;
            }
            case "B" -> config.deptBCrossSelectUrl;
            case "C" -> {
                if (config.deptCCrossSelectUrl == null || config.deptCCrossSelectUrl.isBlank()) {
                    throw new IllegalArgumentException("未配置deptC.crossSelectUrl，但targetDept=C");
                }
                yield config.deptCCrossSelectUrl;
            }
            default -> throw new IllegalStateException("unexpected targetDept=" + targetDept);
        };
    }
